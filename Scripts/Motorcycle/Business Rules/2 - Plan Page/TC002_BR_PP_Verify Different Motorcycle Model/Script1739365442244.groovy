...
TestObject motorSelectionObject = new TestObject('motorSelection')
...
for(int i=0; i<attempt; i++) {
	...
	def locator = "(${GlobalVariable.locator_MotorcycleSelection})[${i+1}]"

	motorObj = WebUI.modifyObjectProperty(motorSelectionObject, 'xpath', 'equals', locator, true)
//	motorSelectionObject.setSelectorValue(SelectorMethod.XPATH, locator)

	WebUI.enhancedClick(motorObj)
...
assert totalPayableAmount.size() == totalPayableAmount.toSet().size() : "Different motorcycle variant have same price: ${totalPayableAmount}"
