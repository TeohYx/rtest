...
TestObject addOnObj = findTestObject('Object Repository/Motorcar and Motorcycle/Plan Page/button_DLSM')
	...
	plateList.each { plate ->
		try {
			...
			if (key == 'negative') {
				...
				if (!isNotPresent) {
					errorLog.add("${plate} is not in legal holder but the addon still present.")
				}
				...
				if (isPresent) {
					String checkedAttribute = WebUI.getAttribute(addOnObj, 'checked')
					
					if (!(checkedAttribute == 'true')) {
						errorLog.add("${plate}: The addon is present, but not ticked by default.")
					}
				}
				
				if (!isPresent) {
					errorLog.add("${plate} is in legal holder but the addon still absent.")
				}
			}
		} catch (Exception e) {
