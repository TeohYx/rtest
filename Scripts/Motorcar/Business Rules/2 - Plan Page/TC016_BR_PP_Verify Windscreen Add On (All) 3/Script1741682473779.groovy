...
TestObject addOnObj = findTestObject('Object Repository/Motorcar and Motorcycle/Plan Page/button_Windscreen')

TestObject windscreenValidation = findTestObject('Object Repository/Motorcar and Motorcycle/Plan Page/1 - Quotation Details/dyvalidation_QuotationDetailsText_text',
	[('text'): GlobalVariable.dyvalidation_addOns['windscreenCoverage']])
...
plateLists.each { key, plateList ->
	plateList.each { plate ->
		try {
			 ...
			 */
			if (key == 'negative') {
				isWindscreenNotPresent = WebUI.verifyElementNotPresent(windscreenValidation, 3, FailureHandling.OPTIONAL)
				...
				isWindscreenPresent = WebUI.verifyElementPresent(windscreenValidation, 3, FailureHandling.OPTIONAL)
