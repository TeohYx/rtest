...
def listInputAmount = CustomKeywords.'utils.Utility.amountDistribution'(rangeOfAmount, contentsInformation.size(), false)
...
rangeOfAmount.eachWithIndex { amount, i ->
	...
	assert totalListInputAmount != amount : "The distribute amount that to be fill in the Contents input are equals to the minimum amount of content amount. It should be 1 unit (1000) different."
	...
	boolean isClickable = WebUI.verifyElementNotClickable(confirmAmountObj, FailureHandling.OPTIONAL)
	
	if (!isClickable) {
		errorLog.add("The object '${confirmAmountObj}' is clickable when it shoudnt be. Amount inputted: ${totalAmount}")
	}	
}
