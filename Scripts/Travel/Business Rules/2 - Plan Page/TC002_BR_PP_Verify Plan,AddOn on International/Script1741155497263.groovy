...
/*
 * When international is select:
 * - Both silver, gold and platinum plan are present
 * - Both add on (adventurous and covid) are present
 * 
 */


def random = new Random()
int tripValue = random.nextInt(2) + 1
int areaValue = random.nextInt(3) + 2

int planValue = 0
if (tripValue == 1) {
	planValue = random.nextInt(4) + 1
} else {
	planValue = random.nextInt(5) + 1
}
...
if (!(WebUI.verifyElementPresent(findTestObject('Travel/TripCare360/English/AppPage/button_AddonCovid19'), 3, FailureHandling.OPTIONAL))) {
